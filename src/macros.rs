...
macro_rules! declare_module {
    (
        ...
        struct $name:ident  {
            $(
                #[in<$in_width:literal> = $btor_in_port:ident, bundle_field = $in_bundle_field:ident]
                $in_field:ident,
            )*
            $(
                #[out = $btor_out_port:ident, bundle_field = $out_bundle_field:ident]
                $out_field:ident,
            )*
        }
    ) => {
    ...
    (
        struct $name:ident  {
            $(
                #[in<$in_width:literal> = $btor_in_port:ident, c_type = $in_c_type:ty]
                $in_field:ident,
            )*
            $(
                #[out = $btor_out_port:ident, c_type = $out_c_type:ty]
                $out_field:ident,
            )*
        }
    ) => {
    ...
    (@__emit_ctor
        struct $name:ident  {
            $(
                #[in<$in_width:literal> = $btor_in_port:ident]
                $in_field:ident,
            )*
            $(
                #[out = $btor_out_port:ident]
                $out_field:ident,
            )*
        }
    ) => {
