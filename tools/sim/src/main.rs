...
struct Args {
    ...
    jit: bool,
    #[arg(long, help = "simulation repetition", default_value_t = 1)]
    repetition: usize,
}
...
fn main() {
    ...
    // let mut average_dirty_percentage: f64 = 0.0;
    let lines = tb.lines().map_while(Result::ok).collect::<Vec<_>>();
    for _ in 0..args.repetition {
        match args.init {
            Init::Zero => {
                sim.init(InitKind::Zero);
            }
        ...
        for (step_id, line) in lines.clone().into_iter().enumerate() {
            do_step(
                step_id,
                &mut *sim,
                &line,
        ...
        for (cell_id, cell) in line.split(",").enumerate() {
            if cell_id == input.0 {
                ...
                if !trimmed.eq_ignore_ascii_case("x") {
                    let value = trimmed.parse::<u64>().unwrap();
                    let width = input.3;
