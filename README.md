...
Currently, only `designs/rocket-latest` is supported. We recommend starting with tests under `riscv-tests/isa` for a quick demonstration of `Patronus` simulator.
Benchmark programs under `riscv-tests/benchmark` might take pretty long time to finish and have not been thoroughly tested.

**Example run**:
```
$ ./resources/emulator -c designs/rocket-latest/TestHarness.btor $RISCV_TESTS/isa/rv64ui-p-add
```
